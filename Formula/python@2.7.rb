class PythonAT27 < Formula
  ...
  skip_clean "bin/pip2", "bin/pip-2.7"
  skip_clean "bin/easy_install2", "bin/easy_install-2.7"
  ...
  def install
    ...
    xy = (buildpath/"configure.ac").read.slice(/PYTHON_VERSION, (2\.\d)/, 1)
  ...
  def post_install
    ...
    %w[setuptools pip wheel].each do |pkg|
      (libexec/pkg).cd do
        system bin/"python2", "-s", "setup.py", "--no-user-cfg", "install",
  ...
  test do
    ...
    system "#{bin}/python#{xy}", "-c", "import gdbm"
